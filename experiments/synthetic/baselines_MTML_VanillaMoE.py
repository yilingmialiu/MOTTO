...
from model.model import Vanilla_MMoE
...
def train_and_evaluate_learner(model, data, num_epochs=60, batch_size=256):
    """Training loop"""
    np.random.seed(RANDOM_SEED)
    torch.manual_seed(RANDOM_SEED)
...
def evaluate_learner_all_scenarios():
    """Evaluate learner on all synthetic scenarios"""
    ...
    for scenario_key, data in scenarios.items():
        try:
            ...
            detailed_results = train_and_evaluate_learner(
                model, 
                data,
...
if __name__ == "__main__":
    ...
    results = evaluate_learner_all_scenarios()

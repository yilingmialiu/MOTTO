...
from model.model import DragonNet
...
def train_and_evaluate_dragonnet(model, data, num_epochs=60, batch_size=512, alpha=1.0):
    """Training loop for DragonNet with propensity loss"""
    np.random.seed(RANDOM_SEED)
    torch.manual_seed(RANDOM_SEED)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_x, batch_t, batch_y in train_loader:
            ...
            pred, prop_scores, _ = model(data_dict)
            ...
            prop_loss = F.cross_entropy(prop_scores, batch_t)
            
            # Combined loss
            loss = pred_loss + alpha * prop_loss
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch}: total_loss = {epoch_loss/num_batches:.4f}, "
                  ...
                  f"prop_loss = {epoch_prop_loss/num_batches:.4f}")
    ...
    with torch.no_grad():
        ...
        all_preds, prop_scores_test, _ = model(data_dict)
        
        # Calculate propensity score accuracy
        prop_accuracy = (prop_scores_test.argmax(dim=1) == T_test).float().mean().item()
        ...
        # Print results
        print("\nPropensity Score Accuracy:", prop_accuracy)
...
def evaluate_dragonnet_all_scenarios():
    """Evaluate DragonNet on all synthetic scenarios"""
    ...
    scenario_params = {
        'A': {'num_epochs': 150, 'alpha': 0.1},  # Less weight on propensity for no confounding
        'B': {'num_epochs': 150, 'alpha': 1.0},  # Equal weight for confounded scenario
    }
    
    print("\nEvaluating DragonNet across all scenarios...")
    print("-" * 60)
    ...
    for scenario_key, data in scenarios.items():
        try:
            ...
            print(f"Epochs: {params['num_epochs']}, Alpha: {params['alpha']}")
            ...
            detailed_results = train_and_evaluate_dragonnet(
                model, 
                data,
            ...
            print(f"Propensity Accuracy: {detailed_results['propensity_accuracy']:.4f}")
...
if __name__ == "__main__":
    results = evaluate_dragonnet_all_scenarios()
    
    # Print summary table
    print("\nSummary of Results")
    print("-" * 100)
    print("Scenario    Description                          Pred MSE    CATE RMSE    Prop Accuracy")
    print("-" * 100)
    for scenario, res in results.items():
        if res is not None:
            ...
            print(f"{scenario:<10} {desc[scenario]:<35} {res['overall_mse']:.4f}      "
                  f"{res['cate_rmse']['average']:.4f}       {res['propensity_accuracy']:.4f}") 